...
//        //logger.info("intializare cu proprietati {}", prop);
    ...
    @Override
    public int getIdCaz(CazCaritabil c) {

        logger.traceEntry("Caut id-ul unui caz {}", c);

        Connection con = utils.getConnection();

        try(PreparedStatement statement = con.prepareStatement("select id from caz where nume = ? and suma_donata =?")){

            statement.setString(1,c.getNume());
            statement.setInt(2,c.getSuma_donata());
            try(ResultSet resultSet = statement.executeQuery()){
                if(resultSet.next()){
                    int id = resultSet.getInt(1);
                    return id;
