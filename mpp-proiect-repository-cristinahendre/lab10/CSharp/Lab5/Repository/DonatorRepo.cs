...
namespace Cerinta1.Repository
{
    public class DonatorRepo : IDonatorRepo
    {
        ...
        {
            log.InfoFormat("se cauta donatorul cu aceste date: {0}, {1}, {2}, {3}",
                nume, prenume, addr, nr);
            ...
            try
            {
                    ...
                    using (var comm = con.CreateCommand())
                    {
                        comm.CommandText = "select id from donator where nume= @nume and prenume=@pre and adresa=@addr and nr_telefon=@nr";
                        ...
                        IDbDataParameter paramAddr = comm.CreateParameter();
                        paramAddr.ParameterName = "@addr";
                        paramAddr.Value = addr;
                        ...
                        if (reader.Read())
                        {
                            ...
                            don.Id = id;
                            log.InfoFormat("am gasit acest id: {0}", id);
            ...
            log.Info("Nu s-a gasit acest donator.");
            return null;
