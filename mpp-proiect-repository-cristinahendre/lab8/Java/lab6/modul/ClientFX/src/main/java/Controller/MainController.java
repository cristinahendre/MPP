...
public class MainController  implements  IObserver {
    ...
    private void setCazuri(){
        ...
        Iterable<CazCaritabil> cazs= service.getCazuri();
        model.setAll((Collection<? extends CazCaritabil>) cazs);
    }

    private void setDonatori(){
        Iterable<Donator> cazs= service.getDonatori();
        modelDon.setAll((Collection<? extends Donator>) cazs);
    }
    ...
    public void refreshCazuri(Donatie cazuri) throws ServiceException {
        Platform.runLater(()-> {
            System.out.println("refresh ctrl");
            setCazuri();
            setDonatori();
            tableView.setItems(model);
                }
